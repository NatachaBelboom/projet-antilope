        ...
        <div class="container">
            <section class="slide-in">
                <div class="titleHeader space">
                    <h2 class="title" role="heading" aria-level="2"><span class="light">Faites une </span>estimation</h2>
                    <span class="line"></span>
                </div>

                <div class="contact__container--estimation">
                    <div>
                        <p>Vous souhaitez connaitre le taux de pollution dans votre commune grâce à nos modules ?</p>
                        <p>Remplissez ce formulaire pour avoir une estimation de prix et vous recevrez une réponse dans les plus brefs délais.</p>
                    </div>
                    <?php if(! isset($_SESSION[$form_type]) || ! $_SESSION[$form_type]['success']) : ?>
                        <form action="<?= get_home_url(); ?>/wp-admin/admin-post.php" method="POST" class="contact__form form--estimation">
                            <?php if(isset($_SESSION[$form_type]) && ! $_SESSION[$form_type]['success']) : ?>
                            ...
                            <div class="form__field">
                                <label for="ministration_number"
                                       class="form__label">Nombre de ministations *</label>
                                <input class="form__input"
                                       type="number"
                                       id="ministration_number"
                                       name="ministration_number"
                                       min="0"
                                       value="<?= noair_get_form_field_value('ministration_number', $form_type) ?>"
                                       placeholder="Nombre de ministration">
                                <?= noair_get_form_field_error('ministration_number', $form_type); ?>
                            </div>
                            <div class="form__field">
                                ...
                                if (($pollution->have_posts())) : while ($pollution->have_posts()) : $pollution->the_post(); ?>
                                    <div class="checkbox__container">
                                        <input type="checkbox" id="pollution" name="pollution[]" value="<?= get_the_ID() ?>">
                                        <label for="pollution" class="form__label--smaller"><?= get_the_title() ?></label>
                                    </div>
                                <?php endwhile; else: ?>
                                <?php endif; ?>
