        ...
        <?php if(! isset($_SESSION['feedback_contact_form']) || ! $_SESSION['feedback_contact_form']['success']) : ?>
            <form action="<?= get_home_url(); ?>/wp-admin/admin-post.php" method="POST" class="contact__form form">
                ...
                <div class="form__field">
                    ...
                    <input type="text" name="firstname" id="firstname" class="form__input" value="<?= noair_get_contact_field_value('firstname'); ?>" />
                    <?= noair_get_contact_field_error('firstname'); ?>
                </div>
                <div class="form__field">
                    <label for="lastname" class="form__label"><?= __('Votre nom', 'dw'); ?></label>
                    <input type="text" name="lastname" id="lastname" class="form__input" value="<?= noair_get_contact_field_value('lastname'); ?>" />
                    <?= noair_get_contact_field_error('lastname'); ?>
                </div>
                <div class="form__field">
                    <label for="email" class="form__label"><?= __('Votre adresse e-mail', 'dw'); ?></label>
                    <input type="email" name="email" id="email" class="form__input" value="<?= noair_get_contact_field_value('email'); ?>" />
                    <?= noair_get_contact_field_error('email'); ?>
                </div>
                <div class="form__field">
                    <label for="message" class="form__label"><?= __('Votre message', 'dw'); ?></label>
                    <textarea name="message" id="message" cols="30" rows="10" class="form__input"><?= noair_get_contact_field_value('message'); ?></textarea>
